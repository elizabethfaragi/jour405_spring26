...
It seems like having a bachelor's degree or higher is correlated with voter turnout, although I will say that all of the voter turnout for most of the counties are pretty high, so I'm wondering if voter turnout in general for Maryland is just high and analyzing it is gonna be hard.  
...
Looking at the scatterplot, none of the variables really seem to predict voter turnout. I'm seeing a lot of clusters of data points in one spot and no real line. None of the variables show data points that actually follow the line of best fit, which leads me to believe that none of these variables are useful. As well, the min and max of residuals are -0.054 and 0.02, which gives a wide spread of variability in both directions. This is concerning to me, even though the adjusted r-squared is 0.8, which accounts for most of the variability in the data.   
...
The full model is still significantly better than any of my models. I think that's always going to be the case because having more variables controlled for/analyzing across multiple variables is always going to account for more of the variability. My best model was model 4, which used population, income, median_age, and pct_white, which used the most variables across my four models, which again furthers my understanding that analyzing across more variables produces a better model. 

```{r}
# Model 2
model2 <- lm(turnout ~ population + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ pct_bachelors_higher + pct_unemployed + income, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ population + income + median_age + pct_white, data = md_turnout)
summary(model4)
...
You could use this analysis to predict why a certain county might have greater voter turnout than a different county based on these variables from the county. You could also compare these predictions across similar counties across the United States and see how voter turnout changes there. 
