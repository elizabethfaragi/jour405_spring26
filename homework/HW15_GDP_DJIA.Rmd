...
gdp_data |> 
  ...
  geom_smooth(aes(x=DJIA, y=GDP, label=Quarter), method="lm") +
  geom_text(aes(x=DJIA, y=GDP, size=5, label=Quarter ))
```
...
There is a really strong  positive correlation between DJIA and GDP. The correlation between them is 0.95, which is a really high and strong correlation. The scatterplot also shows this, as all of the data points are pretty much directly on the line. 
